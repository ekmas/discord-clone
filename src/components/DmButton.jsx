  ...
  return (
    <div className={activeSection === `${chat}` ? 'text-white bg-gray-7 group flex items-center font-medium h-[42px] w-full rounded mb-1' : 'group hover:text-white hover:bg-gray-5 text-gray-3 flex items-center mb-1 font-medium h-[42px] w-full rounded'}>
        ...
        <button onClick={() => {closeConversationButton()}} className='pr-2'>
            <div className={activeSection === `${chat}` ? 'w-4 h-4 opacity-70 m-0.5 pr-2 hidden group-hover:block hover:opacity-100' : 'w-4 h-4 opacity-70 m-0.5 pr-2 hidden group-hover:block hover:opacity-100'}>
            <svg className="closeIcon-1NwtbI" aria-hidden="true" role="img" width="16" height="16" viewBox="0 0 24 24"><path fill="currentColor" d="M18.4 4L12 10.4L5.6 4L4 5.6L10.4 12L4 18.4L5.6 20L12 13.6L18.4 20L20 18.4L13.6 12L20 5.6L18.4 4Z"></path></svg>
            </div>
        </button>
    </div>
  )
